...
function Post() {
    ...
    const fetchPostAndComments = async () => {
      try {
        const [postRes, commentsRes] = await Promise.all([
          axios.get(`http://localhost:4000/api/get/post/${pid}`),
          axios.get(`http://localhost:4000/api/get/comments/${pid}`)
        ])
  ...
  const handleLike = async () => {
    try {
      const response = await axios.put(`http://localhost:4000/api/post/like/${pid}`)
  ...
  const handleAddComment = async () => {
    ...
    setSubmitting(true)
    try {
      const response = await axios.post('http://localhost:4000/api/post/comment', {
        comment: newComment,
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`http://localhost:4000/api/post/delete/${pid}`)
  ...
  const handleEditSave = async () => {
    try {
      const response = await axios.put(`http://localhost:4000/api/post/edit/${pid}`, {
        title: editTitle,
        body: editBody
      })
